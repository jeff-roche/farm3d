...
//! `library.*` events.
//!
//! Fixture substitutions (preflight S6): the Orca fixtures are unavailable,
//! so the "rich" 3MF is `core-two-objects.3mf` rebuilt here with a
//! `Metadata/project_settings.config` part, and the G-code cases use
//! `plain.gcode`, `prusa-cube.gcode`, and an Orca-style file built here.
...
/// project carries (preflight S6's stand-in for `orca-two-plates.3mf`).
fn rich_3mf() -> Vec<u8> {
...
/// An Orca-style G-code file (preflight S6's stand-in for
/// `orca-cube.gcode`): a long comment-only header, as Orca writes its
/// thumbnails first, then a short command body. Its first half has no
/// commands, so a copy truncated to half its length fails inspection.
...
fn gcode_is_retained_byte_for_byte_with_its_claims_untrusted() {
    ...
    let (selection, _) = env.inspected(vec![crlf_source, prusa_source]);
