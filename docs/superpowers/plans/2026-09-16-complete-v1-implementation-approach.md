...
| Geometry renderer | P5 | Accessibility, performance, format, and plate needs — **Resolved (P5)**: see ADR-0010 and the P5 design's D6 and D18. three.js (pinned, lazily loaded) draws meshes that Rust parses with the P4 readers and sends as binary buffers; spike Gate H measured 1 M triangles at a 17 ms median frame in WebKitGTK. Every viewport tool has a numeric and keyboard equivalent, and a text panel replaces the canvas when WebGL is unavailable. Verified in WebKitGTK on Linux x86_64 only. |
| OrcaSlicer runtime contract | P5 | Executable and installed-bundle spike on every F0-declared supported platform — **Resolved (P5)**: see ADR-0009, the P5 design's D2–D11, and the spike report `docs/superpowers/baselines/2026-09-24-p5-orca-runtime-spike.md`. A user-installed OrcaSlicer 2.x (release, prerelease, or nightly) is the engine, with a separate JSON-bearing preset source; one process per plate with fixed arguments; FIFO progress on Linux only. On Linux x86_64, the only F0-declared supported platform, the real-OrcaSlicer tests and tracer pass against v2.4.2, and a packaged farm3d build discovers that engine and extracts its presets. Slicing through the installed package is recorded in the P5 verification record (Task 16b). Windows and macOS stay unverified. |
| Adapter command/camera capabilities | P6 | Real Moonraker/OctoPrint/ElegooLink evidence — **Resolved for Moonraker only (P6)**: see ADR-0011 (composable capability traits), ADR-0012 (the local container simulators are live evidence), the P6 design `docs/superpowers/specs/2026-09-25-p6-connection-command-capabilities-design.md`, the spike report `docs/superpowers/baselines/2026-09-25-p6-moonraker-command-spike.md`, and the verification record `docs/verification/2026-09-26-p6-moonraker-commands.md`. Every Moonraker capability (upload, start, pause, resume, cancel, host state, artifact identity, and the camera query) is `supported` with `tier: sim` evidence citing the committed manifest copy `docs/superpowers/baselines/2026-09-26-p6-sim-manifest-20260926T033455Z.json`. A read-only pass against the owner's Snapmaker U1 (host withheld) adds a host version to the three read capabilities only; farm3d never wrote to real hardware. **Still open:** OctoPrint stays `notVerified` on every capability and needs its own command research (with #10); ElegooLink is out of scope, because #8 closed without a go decision. |
